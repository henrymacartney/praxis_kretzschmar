...
**Document version:** Saturday, 17 May 2026 (end of Alex hero-band image + headline session)  
**Project:** Kretzschmar psychotherapy practice websites  
**Repository:** github.com/henrymacartney/praxis_kretzschmar  
**Local development:** birgitkretzschmar.local / alexkretzschmar.local (Local by Flywheel)

> This document is **complete and self-contained**. It supersedes
> `project_handoff_20260516.md` and all earlier handoffs. No previous handoff is
> needed to have the full picture — everything still current has been carried
...
8. [Current State (As of 17 May 2026)](#8-current-state-as-of-17-may-2026)
...
│   │   ├── project_handoff_20260517.md                 ← this document (authoritative, self-contained)
│   │   ├── project_handoff_20260516.md                 ← prior handoff (superseded)
│   │   └── previous_chat.md                            ← conversation log from Step 10 session
│   ├── text_updates_alex_20260515/                     ← Alex's reworked page texts (Word docs) — pending application
...
│       ├── front-page.php                              ← five-band ACF-driven landing page; hero band restructured 17 May (4-line headline)
│       ├── impressum_datenschutz.html                  ← Alex old-site legal source (for real Impressum/Datenschutz build)
│       ├── screenshot.png                              ← WP theme thumbnail
│       ├── acf-json/                                   ← per-site ACF field group exports
│       │   ├── README.md                               ← documents the field groups
│       │   ├── group_69fea7e373804.json                ← Alex Hero (hero_line3/hero_line4 added 17 May)
│       │   ├── group_69feab085e4c6.json                ← Alex Leistungen Grid
│       │   ├── group_69feaf601eb22.json                ← Alex Über mich Band
│       │   ├── group_69feb15a3dd04.json                ← Alex Kontakt CTA Band
│       │   ├── group_69feb3501eac1.json                ← Alex Footer
│       │   ├── group_6a01e163bed1f.json                ← Alex Über mich page
│       │   ├── group_6a02a5f5b7be7.json                ← Alex Kontakt page
│       │   └── ui_options_page_69feb70243efd.json      ← Footer options page
│       ├── assets/icons/                               ← inline-SVG icons for the 6 Leistung cards
│       │   ├── README.md                               ← Heroicons-compatible, currentcolor convention
│       │   ├── tiefenpsychologie.svg / gestalttherapie.svg / psychoonkologie.svg
│       │   └── hypnotherapie.svg / coaching.svg / praxis.svg
│       ├── assets/images/
│       │   ├── consultation_landscape_bw.jpeg          ← NEW 17 May — desktop hero (grayscale)
│       │   ├── kretzschmar-1-scaled_bw.jpg             ← NEW 17 May — mobile/tablet hero (grayscale)
│       │   ├── consultation_landscape.jpeg             ← colour source (desktop hero)
│       │   ├── kretzschmar-1-scaled.jpg                ← colour source (mobile/tablet hero)
│       │   ├── consultation_bw_web.jpg                 ← OLD desktop hero (superseded 17 May, retained for roll-back)
│       │   ├── consultation_mobile_bw_web.jpg          ← OLD mobile hero (now unreferenced, retained)
│       │   ├── consultation_tablet_bw_web.jpg          ← OLD tablet hero (now unreferenced, retained)
│       │   ├── consultation.jpg / consultation_bw.jpg / consultation_mobile.jpg  ← earlier sources
│       │   ├── consultation_hero_image_notes.md        ← hero image pipeline notes
│       │   ├── alex_kretzschmar_portrait.jpg           ← Über mich portrait (80 KB re-encode)
│       │   └── logo_alex.png / logo3.jpg / logo3-150x150.jpg  ← logo assets
│       ├── template-parts/
...
- `docs/handoffs/project_handoff.md` — handoff document (this lineage;
  later renamed/dated, current version is `project_handoff_20260517.md`)
...
### 17 May 2026 session — Alex landing-page hero band: image swap + headline restructure

**Scope:** Alex's site only (`shared/themes/alex_child/`). Birgit's
site not touched. `resources/` not touched. No Git run by the AI
assistant (per CLAUDE.md, Henry runs all Git).

**Hero background images changed:**

- **Desktop hero** → `consultation_landscape_bw.jpeg` (grayscale,
  1774×887, ~188 KB), derived from the supplied
  `consultation_landscape.jpeg` using the documented Pillow pipeline
  (`convert("L")`, Lanczos, JPEG q82, progressive, optimize). Set via
  the `hero_background_image` ACF field on the Startseite (page ID 8),
  "Alex Hero" group.
- **Mobile + tablet hero** → `kretzschmar-1-scaled_bw.jpg` (grayscale,
  2560×2041, ~272 KB), derived from the supplied
  `kretzschmar-1-scaled.jpg`. A **single file serves both
  breakpoints**. This is a deliberately different, more
  portrait-suited photograph: the landscape image cropped badly on
  narrow viewports. Same landscape/portrait pairing rationale as the
  earlier `consultation_bw_web.jpg` / `consultation_mobile_bw_web.jpg`
  pattern.
- Wired via the two `@media` blocks in
  `shared/themes/alex_child/tailwind.css` (mobile ≤768px; tablet
  769–1024px), each `url()` repointed to `kretzschmar-1-scaled_bw.jpg`.
  Only those two lines changed in `tailwind.css`. The hero treatment
  (`filter: grayscale(100%) brightness(0.55)` inline in
  `front-page.php`; `opacity: 0.2` and parallax in the tailwind.css
  "Item 12" block) was left unchanged.

**Hero text restructured (front-page.php hero band only):**

- The single headline + subheadline was replaced with **four stacked,
  centered, cream lines** in descending size/weight, reflecting the
  practice letterhead hierarchy:
  1. `Psychotherapeutische Praxis` (largest, `<h1>`)
  2. `Dr. rer. physiol. Dipl.-Psych. Alexander Kretzschmar`
  3. `Psychologischer Psychotherapeut`
  4. `Nußbaumstrasse 5 / 65187 Wiesbaden / Tel. 0611/846840` (smallest)
- Kept **centered** (not left-aligned) per Henry's decision; existing
  hero CTA buttons retained.
- Two new ACF text fields `hero_line3` and `hero_line4` added to ACF
  group `group_69fea7e373804.json` ("Alex Hero", page 8), between
  `hero_subheadline` and the CTA fields. `front-page.php` reads them
  with presence guards so empty fields render nothing. Group key,
  title, and page-8 location rule unchanged; `modified` timestamp
  bumped.

**Verified by Henry (17 May 2026):** ACF Sync of the "Alex Hero"
group, wp-admin entry of the four hero text lines on the Startseite,
and CSS recompilation via the running `tw:watch` watcher — all
confirmed done successfully.

**Files changed this session (in `shared/themes/alex_child/`):**

- `front-page.php` — modified (hero band only: +2 ACF reads, hero text
  block replaced)
- `tailwind.css` — modified (two `@media` `url()` lines)
- `acf-json/group_69fea7e373804.json` — modified (+`hero_line3`,
  +`hero_line4`, timestamp bump)
- `assets/images/consultation_landscape_bw.jpeg` — new
- `assets/images/kretzschmar-1-scaled_bw.jpg` — new

The previous hero files (`consultation_bw_web.jpg`,
`consultation_mobile_bw_web.jpg`, `consultation_tablet_bw_web.jpg`)
remain on disk, retained for roll-back; they become unreferenced after
this change.

**Alex review feedback received (pending — next session):** Alex
delivered reworked page texts as Word documents in
`docs/text_updates_alex_20260515/`, and gave feedback on the new hero
image. Applying the text updates and the next round of hero-band
re-arrangement are the subject of the next working session; their
content is not yet reviewed or applied here.
...
- ACF Pro Freelancer license (paid by Henry -> charged to Beriott GmbH -> 
  customer)
- Activated on Birgit's local site
- Activated on Alex's local site
- Activated on Hostinger sites (Birgit & Alex)

### Production (planned and provisioned)

- Hostinger Premium for Birgit (provisioned)
- Hostinger Premium for Alex (provisioned)
- Domains stay with Strato (DNS pointing to Hostinger after cutover)

### Outlook/Microsoft 365 for email

- Birgit and Alex use existing email accounts - MS Outlook? (not part of this 
  project)
- Critically: Alex's Kassenpraxis correspondence uses KV-SafeNet — must NOT be
  touched and not part of this project
...
## 8. Current State (As of 17 May 2026)
...
- **Startseite** — five-band landing page:
  - Hero band: as of **17 May** the desktop background is
    `consultation_landscape_bw.jpeg` (grayscale landscape) and the
    mobile/tablet background is `kretzschmar-1-scaled_bw.jpg`
    (grayscale, more portrait-suited, single file for both
    breakpoints, set via two `@media` blocks in `alex_child/tailwind.css`).
    Treatment unchanged: parallax (`background-attachment: fixed`),
    `opacity: 0.2` ("Item 12" tailwind.css block), grayscale +
    brightness 0.55 filter (inline in `front-page.php`), navy gradient
    veil. Hero text is now **four centered cream lines** (practice
    name / Dr. name+title / "Psychologischer Psychotherapeut" /
    address line) driven by ACF fields `hero_headline`,
    `hero_subheadline`, `hero_line3`, `hero_line4`; two hero CTAs
    retained below.
...
### Git working-tree state (as of the 17 May `git status`)

Branch `main`, up to date with `origin/main` at last push. Working tree
contains **two logical change sets** not yet committed:

- **Staged (from the prior session):** `CLAUDE.md` modified; handoff
  and Birgit text-update doc renames/moves
  (`docs/handoffs/project_handoff_20260512.md`,
  `docs/text_updates_birgit_20260514/...`); several `alex_child` asset
  additions (`consultation_landscape.jpeg`, `consultation_mobile.jpg`,
  `consultation_mobile_bw_web.jpg`, `consultation_tablet_bw_web.jpg`,
  `kretzschmar-1-scaled.jpg`, `logo3-150x150.jpg`, `logo3.jpg`),
  `impressum_datenschutz.html`, `screenshot.png`.
- **Not staged (this session's hero work):**
  `shared/themes/alex_child/acf-json/group_69fea7e373804.json`,
  `shared/themes/alex_child/front-page.php`,
  `shared/themes/alex_child/package.json`,
  `shared/themes/alex_child/tailwind.css`.
- **Untracked:** `docs/handoffs/project_handoff_20260516.md`,
  `docs/text_updates_alex_20260515/`,
  `shared/themes/alex_child/assets/images/consultation_landscape_bw.jpeg`,
  `shared/themes/alex_child/assets/images/kretzschmar-1-scaled_bw.jpg`,
  and this document (`docs/handoffs/project_handoff_20260517.md`, once
  placed).

**Nothing from the 14 May or 17 May sessions is committed or pushed
yet.** Git is Henry's responsibility (CLAUDE.md). Henry decides whether
to commit the prior staged set and the 17 May hero work together or as
separate commits.

**Database (Alex):** changed this session — `hero_background_image`
re-pointed and the four `hero_*` text fields set on the Startseite
(page ID 8), via wp-admin; ACF "Alex Hero" group Synced. Verified by
Henry. (On Local only — not migrated anywhere.)

**Database (Birgit):** changed in the 14 May session — Über mich (bio
...
renamed/split. Harmless (nothing reads it). The 17 May Alex change
introduces no orphaned data (fields added, not renamed).
...
**17 May 2026 (most recent):** The Alex landing-page hero band was
changed — new desktop image (`consultation_landscape_bw.jpeg`), new
mobile/tablet image (`kretzschmar-1-scaled_bw.jpg`), and a four-line
centered cream headline driven by ACF (`hero_line3`/`hero_line4` added
to the "Alex Hero" group). Applied and **verified by Henry** on Alex's
Local site (ACF Sync + wp-admin field entry + `tw:watch` recompile).
Alex has also delivered reworked page texts as Word documents in
`docs/text_updates_alex_20260515/` and feedback on the new hero image.

Pause point (17 May): hero changes applied and verified; the working
tree carries the prior staged set **plus** this session's hero work,
about to be committed by Henry. **Applying Alex's text updates and the
next round of hero-band re-arrangement (per his image feedback) are the
subject of the next working session** — not started, content not yet
reviewed.

The 14 May context below remains accurate and still-open:
...
Pause point (14 May): **theme + ACF + doc changes are staged but not yet
...
commit, push, then AIO-migrate Birgit's updated Local build to her
demo domain so the Kretzschmars see the revised content.
...
### Immediate (post-17-May session, in order)

1. **Henry: commit and push** the current working tree (prior staged
   set + the 17 May Alex hero work). Henry decides whether to commit
   together or split into separate logical commits. Alex's text-update
   Word docs in `docs/text_updates_alex_20260515/` and the
   `project_handoff_20260517.md` should be included.
2. **Next session — apply Alex's reworked page texts** from
   `docs/text_updates_alex_20260515/` across his site pages.
3. **Next session — re-arrange the hero-band section** per Alex's
   feedback on the new hero image (specifics to be defined at the start
   of that session).
4. **Migrate Birgit's updated Local build to the Hostinger demo**
...
5. **Run the Tailwind build** (or confirm the watcher ran) so the
   prose CSS fix is in `build/theme.css` before the migration export
6. **Send Alex the demo** for his review (URL/screenshots/Loom or
   point him at his Hostinger demo domain behind basic auth)
7. **Investigate Tailwind `md:hidden` scan-path issue** — fix
   properly so future `md:*` utilities work without explicit-CSS
   workarounds
8. **Cleanup Sample Page and Privacy Policy drafts** in Alex's
   wp-admin
9. **Real Impressum and Datenschutz on Alex's site** — reuse his
   old-site versions (source in `alex_child/impressum_datenschutz.html`)
   minus the Google Maps embed paragraph
...
**Resolved 17 May 2026:**

- *Alex hero image* — decided and applied: grayscale landscape
  consultation image for desktop + grayscale Kretzschmar image for
  mobile/tablet (single file, both breakpoints).
- *Alex hero text layout* — decided: four centered cream lines
  (practice name / Dr. name+title / role / address), CTAs retained,
  centered (not left-aligned).

**Open 17 May 2026 (deferred to next session):**

- Exact nature of Alex's requested hero-band re-arrangement (depends
  on his image feedback — to be reviewed at the start of the next
  session).
- How Alex's text-update Word documents in
  `docs/text_updates_alex_20260515/` map onto his existing page
...
- `docs/handoffs/project_handoff_20260517.md` (this document — complete
  and self-contained, covers everything)
...
> project state is in `docs/handoffs/project_handoff_20260517.md`. Read
> that first.
>
