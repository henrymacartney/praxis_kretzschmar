...
$recurring  = get_field( 'termine_wiederholend' );
...
/**
 * Render a single recurring termin entry (no date — uses Wiederholung text instead).
 *
 * @param array $entry Repeater row from termine_wiederholend.
 * @param array $gruppentyp_labels Group-type label map.
 */
function praxis_render_recurring_entry( $entry, $gruppentyp_labels ) {
	$titel        = isset( $entry['titel'] ) ? $entry['titel'] : '';
	$wiederholung = isset( $entry['wiederholung'] ) ? $entry['wiederholung'] : '';
	$ort          = isset( $entry['ort'] ) ? $entry['ort'] : '';
	$gruppentyp   = isset( $entry['gruppentyp'] ) ? $entry['gruppentyp'] : '';
	$beschreibung = isset( $entry['beschreibung'] ) ? $entry['beschreibung'] : '';
	$kosten       = isset( $entry['kosten'] ) ? $entry['kosten'] : '';
	...
	<article class="border-l-2 border-cream-200 pl-6 py-4">
		...
		<?php if ( $wiederholung ) : ?>
			<p class="font-display text-xl text-navy-800 mb-1">
				<?php echo esc_html( $wiederholung ); ?>
			</p>
		<?php endif; ?>
		...
		<section class="py-12 md:py-16 bg-cream-50">
			<div class="mx-auto max-w-3xl px-6">
				...
				<?php if ( ! empty( $recurring ) ) : ?>
					<h2 class="font-display text-2xl md:text-3xl text-navy-800 mb-8 mt-16">
						Regelmäßige Termine
					</h2>
					<div class="space-y-8">
						<?php foreach ( $recurring as $entry ) : ?>
							<?php praxis_render_recurring_entry( $entry, $gruppentyp_labels ); ?>
						<?php endforeach; ?>
					</div>
				<?php endif; ?>
